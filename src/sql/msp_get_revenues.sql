...
BEGIN
     ...
     WHERE l.lagerQuality = 0
          AND lc.disableRow = 0
          AND lg.businessId = 1                 --Сильпо
          AND lg.commodityGroupId in (5550002, 5550106, 5550014, 5550005, 5550121, 5550930, 5550008)'
     ...
     WHERE l.lagerQuality = 0
          AND lc.disableRow = 0
          AND lg.businessId = 1                 --Сильпо
          AND lg.commodityGroupId in (5550118, 5550150, 5550151, 5550499)              '
     ...
     -- Подливаем выручку второй волны
     IF @Date > '20210412'
     BEGIN
          INSERT INTO #Sales
          SELECT
          ...
          FROM Cheques.dbo.view_ChequeLines cl with (nolock)
               JOIN #Lagers l
                    ON l.LagerId = cl.LagerID
                         AND wave = 2
          WHERE created BETWEEN '20210412' AND @Monday
               AND filid IN (
                    2382, 2129, 2236, 2251, 2085, 2056, 2045, 2025, 1934,
                    1995, 2042, 2145, 2183, 2173, 2197, 2009, 2132, 2071, 2051, 2275, 2121,
                    2261, 2144, 2192, 2107, 2123, 2105, 2087, 2273, 2247, 2226, 2149, 2232, 2052, 1998, 2256, 2187, 2031, 2122, 2268, 2015,
     ...
     -- Подливаем выручку третьей волны
     IF @Date > '20990101'
     BEGIN
          INSERT INTO #Sales
          SELECT
          ...
          FROM Cheques.dbo.view_ChequeLines cl with (nolock)
               JOIN #Lagers l
                    ON l.LagerId = cl.LagerID
                         AND wave = 3
          WHERE created BETWEEN '20990101' AND @Monday
               AND filid IN (
                    2382, 2129, 2236, 2251, 2085, 2056, 2045, 2025, 1934,
                    1995, 2042, 2145, 2183, 2173, 2197, 2009, 2132, 2071, 2051, 2275, 2121,
                    2261, 2144, 2192, 2107, 2123, 2105, 2087, 2273, 2247, 2226, 2149, 2232, 2052, 1998, 2256, 2187, 2031, 2122, 2268, 2015,
